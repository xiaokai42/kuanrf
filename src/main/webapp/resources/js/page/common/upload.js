 ...
 * @param inputId 保存信息
 */
function uploadVedio(type, fileId) {
	$("body").block({message:"文件上传中..."});
	$.ajaxFileUpload({
		type : "post",
		url : Util.getRootPath()+"/public/upload/video",
		secureuri : false,
		dataType : "JSON",
		fileElementId : [fileId], // 文件上传的id属性
		data : {
		...
		success : function(text, status) {
			...
			if (data.flag) {
				var src = Util.getRootPath()+"/public/showPhoto?id="+data.id;
				$("#"+fileId + "-video").attr("src", src);
				$("#"+fileId + "-input").val(data.id);
				$("#"+fileId).replaceWith('<input type="file" id="'+fileId+'" data-type="'+type+'" class="uploadImg" name="file" accept="video/*" />');
			} else {
				layer.msg(data.msg, {time: 5000, icon:5});
			}
		},

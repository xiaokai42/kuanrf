...
function initButton() {
	...
	$("#add-btn").on("click", function () {
		window.location.href =  Util.getRootPath() + "/recruit/toAdd/0";
	});
	...
	$("#edit-btn").on("click", function () {
		...
		if (datas.length <= 0) {
			...
			window.location.href = Util.getRootPath() + "/recruit/toAdd/" + datas[0].id;
		}
	});
    ...
    $("#img-btn").on("click", function () {
		layerObj = top.layer.open({
			...
			type: 2,
			area: ['600px', '200px'],
			content: [Util.getRootPath() + '/recruit/editImg', 'no'],
			closeBtn: 1
		});
    });
...
function initDataGrid() {
    grid = Util.createTable({
        loadURL: Util.getRootPath() + "/recruit/list/page",
        ajaxLoad: true,
